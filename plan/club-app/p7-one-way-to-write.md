...
*Gruppe bearbeiten*, an inline panel two thousand pixels down the Gruppe hub for the six-field
Steckbrief, an inline row editor that deletes the row it edits in the Personenverwaltung, a bottom
sheet for the Aushang, and bare controls that commit on change on `/manage/board` and `/profile`.
...
nothing.** Tapping *Pflegen* at the top of the Gruppe hub opens an editor 2 400 px below on a
...
3. **Ending is an entry write, not a confirmation.** The six *beenden* dialogs go; ending a
   Mitgliedschaft is filling in its end date in the entry editor, with the consequence line the
   editor already owes.
...
8. **Peek sheets stop carrying write actions.** `HubPeekSheet` becomes *this person, in this
   Gruppe* — her Zugehörigkeit and her Gruppen-Admin-Ernennung as rows that open their editors.
...
14. **The Gruppenverwaltung register links to the hub** — ADR-0014's
    [2026-09-22 amendment](../../docs/adr/0014-the-gruppe-hub-writes-its-own-record.md). Rows lose
    their action cluster, the club's acts move into a block at the end of the hub shown only to a
    holder of `groups.manage`. One named exception: an archived Gruppe has no hub, so *Zurückholen*
    stays on its row.
15. **The Trainingsgenerator is not an edit mode.** It is a bulk write with a preview, it already
...
stay confirmations, moved off their rows: the four Archive/Restore pairs (Vorstandsfunktion,
Gruppe, Gruppenart, Ort), the unpaired `ArchiveRoleDialog`, plus `WithdrawAnnouncementDialog`,
...
all three parts of the test. `RolePermissionRow` is a switch and **stays**: a Rolle's permission set
...
**W1 — the Gruppe hub.** The reference implementation, and first because it carries four of the
five old paradigms on one screen. `HubCarePanel`'s inline editor becomes a detail write at
`/groups/$groupId/bearbeiten`; the duplicate *Pflegen* on `HubDescriptionPanel` goes with it and
...
kind. `HubPeekSheet` becomes the person's screen within the Gruppe (ruling 8) and loses its
...
**W2 — the Personenverwaltung.** The worst offender for data, because `FactEditorFrame` replaces
...
`EndMembershipDialog` folds into the Mitgliedschaft editor. `PersonFormDialog` becomes a detail
...
new *für die Verwaltung* block carries Name, Gruppenart and *Gruppe archivieren* (ruling 14).
`BoardOfficeRoleField` loses its autosave. The `nur Ansicht` chips go (ruling 13).

**W4 — Kalender and Aushänge.** `CalendarEntryFormDialog` — eleven fields in a dialog whose footer
...
- **Concurrency.** Two Gruppen-Admins editing the same Gruppe is last-write-wins today and stays
...
- **The Trainingsgenerator's preview** (ruling 15). It works; it moves container and nothing else.
...
- **Concurrency.** Unanswered, deliberately. The question is not technical — it is what the club
  wants a second Gruppen-Admin to be told when her save would overwrite one made four minutes ago.
...
- **The archived-Gruppe exception** (ruling 14) is the only row in the app that keeps a button. If
