...
One event's own page: the key facts (date, Einlass, Beginn, venue), what the evening is,
...
- Tag row (type · session · age hint · venue) + big derived stat row (date, Einlass,
  Beginn, Ende, price)
- Ticket panel: price, live capacity bar, state-dependent CTA (choose seat / waitlist
  when sold out / reminder when presale not open), payment hints, group order note
- "So war es letztes Jahr" photo theater — an auto-advancing photo stack pulled from the
  gallery, with pause + dots
- "Wer auftritt" group chips · running order block (one honest line while unpublished; a
  collapsible timed rundown once it exists; "live" mode teaser for the evening itself)
- hall info card, "Auch noch in dieser Session" cards, closing band with countdown + CTA

## Decisions

Carried from the events-list shaping (2026-08-13):

- **Route is `/events/$eventSlug`, slug = the seed's event id** (gallery precedent,
...
- **Routes pre-decided by those CTAs:** **`/events/$eventSlug/seats`** (seat picker —
  ...
  (ticket exchange — its own destination, kept inside the area; static route beats
...
- **`/tickets` is retired in E3.** It was a live `PlaceholderPage` no page plan owned, and
  ...
  ticket starts by choosing an evening, so the list *is* the ticket entry. Glossary gained
  **ticket**, **presale**, **ticket exchange**.

### The running order on a public page

- **The running order exists late.** The responsible person assembles it roughly **two to
  three weeks before** the evening (user, 2026-08-15), so for most of an event's public
  life there is none. Recorded in `CONTEXT.md`.
- **Consequence for the design: the running order is an addon, never the page's spine.**
  The page must read complete without it; when it exists the section is a modest secondary
  block, and when it doesn't the section is simply **absent** — no empty box, no
...
- **Published: the order, never the times.** The sequence is stable enough to print, the
  ...
  **rejected** (design fiction; the live running order is Club-App territory if it ever
  exists).
- **Acts are free-text names.** Groups *and* guests appear (guest clubs, external
  speakers), so acts travel as names, not group ids. A group reference is reserved in the
  contract; nothing links in E3 — a lineup that links half its entries reads broken.
- The word **"Programm"** never appears; the section header is **"WER AUFTRITT"**.

### Model additions

- **`performers`** — the running order's public face: an ordered list of act names,
  nullable until the running order is assembled. Progressive publishing, same rule as
  everything else.
- **`description`** — an optional longer per-event text (plain paragraphs). Without it, an
  evening with no running order yet and no matching Album (e.g. `weiberfasching-2027`)
  would be tags, one teaser sentence and a panel. Absent → the identity block falls back to
  the teaser and the page still reads complete.
- **`eventType` on Album** (gallery seed) — the explicit key for the photo match below.

### Photos from last year

- **Matched by event *type*, never by event id.** The glossary is explicit that an Album is
  not bound to an event (an Album may cover an unticketed occasion), and matching by
...
- **A static preview (3–4 photos) linking into the Album** — the mock's auto-advancing
  photo theater is **rejected**: motion for its own sake, duplicating the viewing experience
  the gallery already owns. Section is absent when nothing matches (3 of the 6 seed
  evenings today).
...
panel · "Wer auftritt" (when present) · Album preview (when matched) · hall · "Auch noch in
dieser Session" · closing band.

- **The hall block is E2's `VenueBlock`, promoted to a feature-shared component** and
  rendered on both pages — one home for the placeholder hall facts, never two copies. The
...
| `salesClosed` | "Vorverkauf beendet", no CTA — no door-sales claim (unknown) |
| `cancelled` | "Abgesagt": no capacity bar, no countdown, no CTA (tests only, never seeded) |

- **No presale reminder capture.** An email field that stores nothing is exactly the dishonest
...
- The mock's payment hints ("Kreditkarte, PayPal — oder bar im Vereinsraum"), the
  **group order** note ("X sammelt für Reihe 4") and the **LIVE-KARTENSTAND** label
...
- **The `/events` ticket-exchange band gains its link** ("Zur Kartenbörse →", still zero
  mechanics claims). Otherwise the site holds two truths about whether the ticket exchange
  is reachable.
...
- Route `routes/_site/_gated/events_.$eventSlug.tsx` gets a **loader** resolving the event
  ...
  guard as E2: placeholder key facts must never reach rich results — the gate holds until
...
- Whether tickets are also sold at the door — until answered, `salesClosed` says nothing.

## Ideas parked (not scheduled)

- **Presale reminder** — notify people when a presale opens. Needs a backend (email
  capture, consent, sending) and a decision that the club wants to send mail at all.
- **Running order act → group deep links** once acts carry an optional group reference.
- **Live running order on the evening itself** — only if the Club-App ever publishes one.
...
- **E3 — Event page** (this file, four slices, one phase; per-slice reviews and
  ...
  1. **Model + entry sweep** — `performers` + `description` on the event seed (placeholder
     ...
     `/events/exchange` placeholder routes; the `/events` ticket-exchange band linked.
...
- **The masthead carries one events entry, not two** (user ruling during the build).
  ...
  separate tickets and events pages"*. So **`/events` left `navItems`** and became the
...
- **Album `eventType` is a typed optional field, not Zod-parsed.** The Done-When asked for it,
  but the gallery's content is a compile-time TS constant with no schema anywhere (P5/ADR-0003
...
- **`performers` are the club's real groups only.** No guest acts were invented: an invented
  guest-club name would be a plausible-sounding lie (the P5 photo-credit / P6 Jeck-Check rule).
...
- **The Album preview lives in the gallery feature** (`AlbumPreview`), not in events — it
...
- [Area plan](master-plan.md) · [Foundation](foundation-events-data.md) ·
  [Event list](page-event-list.md)
- [Gallery](../feature-gallery.md) — session derivation + photo source precedent
- [`CONTEXT.md`](../../../CONTEXT.md) — Event, **Running order** (late, order-only),
  **Ticket**, **Presale**, **Ticket exchange**
