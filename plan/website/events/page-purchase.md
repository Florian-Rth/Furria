...
From "Deine Daten" to an order in hand: the buyer form (step 2), the order summary +
payment (step 3), and the confirmation page at `/orders/$orderCode` the mail links to. **The
...
(Stripe-only, embedded); and a second undecided club fact surfaced (**entry check**,
`CONTEXT.md`), which blocks the digital ticket's face the way seat allocation blocks the
ticket selection.
...
- "Was jetzt passiert" steps (mail with QR, reminder two days before, live running order on the
  evening)
- Return/swap teaser into the ticket exchange, cross-sell card for the other open evening
- Payment mentions across the mock: credit card, PayPal, "reservieren und bar im
...
1. **Guest checkout reconfirmed** (challenged and upheld): buying never requires an
   Account; `/orders/$orderCode` stays the guest's retrieval path; the ticket overview is
   the *additional* view Account holders get later. `CONTEXT.md` (Account, Order)
   stands unchanged.
2. **One buyer per order — Vorname, Nachname, E-Mail. Nothing else.** No phone, no
   postal address, no per-ticket attendee names (anti-scalping personalization is friction
   with no threat model here, and GDPR minimization wins).
3. **Stripe is the single payment provider for ticket sales** — this *amends* the carried
   stub decision "Stripe (card) + PayPal" (design §10): the concrete methods (card,
   PayPal, Apple/Google Pay, …) are whatever the club enables in its Stripe config; the
   website never hand-maintains a method list. Scope is **tickets only** — other money
   flows (fee, shop, drinks till) stay open, so no ADR.
...
6. **Entry check is undecided club practice** (`CONTEXT.md` flagged ambiguity,
   ...
   page shows the order honestly without codes; the mock's `EvTicket` QR face is
   embargoed, not rejected.
7. **The confirmation page `/orders/$orderCode` is the digital ticket** — four blocks:
   state hero → order summary → cross-sell (another purchasable evening, derived from
   the seed) → ticket exchange existence teaser (links the live placeholder route, says only
   *that* it exists — glossary rule). **Cut:** reminder-mail-two-days-before (unverified
   practice, unbuilt) and live-running-order-on-the-evening (event-app territory) promises. The
   "mail ist unterwegs" line is safe: the backend that creates the order is the thing
   that sends the mail.
8. **The order payload carries a `paymentStatus`** — `paid | processing` — because
...
9. **The capability URL shows buyer name *and* email** (user decision): the link-equals-mail
   ...
   protects nothing.
10. **No terms-and-conditions checkbox in this build.** Ticket terms and conditions do not exist; a consent checkbox linking
    to nothing would be a dead link *inside a legal act*. **Ticket terms and conditions are flagged as an
    open club/legal fact and a blocker for real online sales** (deferred contract). The
    **right-of-withdrawal exemption notice ships now** (§312g Abs. 2 Nr. 9 BGB — dated leisure
...
12. **Step 3 ships final-except-blocked**: real buyer summary (from step 2's state) + the
    right-of-withdrawal notice + a **visibly-placeholder ticket line** (the selection seat allocation
...
13. **`/orders/demo` renders the real confirmation page** on a seeded, **visibly fake**
    demo order (invent-visibly rule) via the normal query hook; **any other
...
- **Entry check** → the ticket face (QR per ticket? name list?), PDF/Wallet/calendar
  artifacts, and the eventual event-app scanner. (`CONTEXT.md`, 2026-08-19)
- **Seat allocation** → step 3's real ticket line + sum, and everything E4 lists for
  step 1. (`CONTEXT.md`, 2026-08-18)
- **Ticket terms and conditions** → the consent checkbox + `/agb` legal page; blocker for real online
  sales, not for this build.

## Done When

- `/orders/demo` renders the final confirmation page from the seeded demo order;
...
- Step 2 is a fully functioning validating form; invalid data blocks progression; the
  typed values appear in step 3's order summary.
- Step 3 carries the right-of-withdrawal exemption notice and the §312j summary framing; the ticket
...
1. **S1 — Order model + seed + route hygiene**: `lib/seed/orders.ts` (Zod schema:
   ...
   order), the `events` feature's order query hook (demo resolves, unknown codes
...
2. **S2 — Step 2 "Deine Daten"**: the final RHF + Zod buyer form, flow-local state,
   ...
   code comment.)
3. **S3 — Step 3 "Zahlung"**: order summary (real buyer data, visibly-placeholder
   ticket line), right-of-withdrawal notice, updated placeholder payment region, CTA semantics.
...
- **The §312j information set is deliberately incomplete, and guarded.** Summary and right-of-withdrawal notice
  ship on step 3; the **total price does not**, because decision 12's visibly-placeholder ticket
  line means the count is unknown. The copy says so out loud ("Solange fehlt auch die Summe") and
  `buildOrderDraftSummaryRows` provably omits tickets/total. "Zahlungspflichtig bestellen" may not
...
- **Three placeholder regions ship, not the two the plan names**: step 1's selection core
  (E4-sanctioned), step 3's ticket line (a consequence of step 1, decision 12) and step 3's payment
...
- **The right-of-withdrawal notice is a `role="note"`, never `role="alert"`** — a permanent legal statement must
...
- **One seed copy edit crossed phases:** the Rentnerfasching description lost "mit mehr Platz
  zwischen den Reihen" (it encoded a seat allocation variant on E3's shipped page) for "mit mehr
...
- **The strongest honesty tension in E5 is deliberate:** `/orders/demo` shows a BEZAHLT hero for
  ...
  visibly-fake demo order and forbids a second code; the **BEISPIELBESTELLUNG** band
...
- [Area plan](master-plan.md) · [Ticket order flow](page-order-flow.md) ·
  [Ticket exchange](page-ticket-exchange.md)
- [`CONTEXT.md`](../../../CONTEXT.md) — Order, Ticket, Account, Ledger +
  Seat allocation / Entry check / Guest-registration flags
