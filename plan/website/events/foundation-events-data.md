...
truth (the move P6 made for `/club`'s groups).

## Mock inventory (inspiration only)

- `EV_EVENTS` — the event model: date/time fields, Einlass/Ende, type, capacity, free
  count, status (`offen · knapp · ausverkauft · bald`), presale start date, age hint, teaser
...
1. **Seed module** — `src/lib/seed/events.ts`: `SalesStatusSchema` (7 public values),
   ...
   contradict themselves, and `SEEDED_EVENTS` (six evenings, mid-presale snapshot, no
...
2. **`events` feature scaffold** — `features/events/` with `api.ts`
   (`useEventsQuery` + `eventKeys`, groups pattern) and the pure presentation
...
3. **Teaser rewire** — the landing **"Programm" teaser** consumes the seed through its own thin
   hook; the Rose Monday parade and extra venues drop out; `PROGRAM_EVENTS` is deleted.
...
- Consumes **published key facts** from the future backend (public read); the website never
...
- **The model covers ticketed evenings only.** **"Programm"** (website sense) was narrowed in
  `CONTEXT.md`: unticketed happenings (the Rose Monday parade) are not part of the event list and
  not listed on the website. Consequence: the shipped `PROGRAM_EVENTS` teaser constant
  (`landing/program-content.ts`) currently lists a Rose Monday parade at the village square and
...
- **Seeds show a plausible mid-presale snapshot** (mix of presaleScheduled → soldOut states),
...
- **Ids/slugs follow the gallery precedent** — German domain words + year
  (`prunksitzung-1-2027`), matching `prunksitzung-2026` album slugs.
- **Event data is consumed the groups way** — one `src/lib/seed/events.ts` module; each
...
- **Key facts verified per field:** Einlass + Beginn times, an age hint and a teaser
  sentence + event type are real published key facts. The mock's rough end time
...
- **Motto is per Session, not per event** — no motto field on the event model. The master
  plan's key-facts list ("name/date/location/motto") reads motto as session-level.
- **Sales status is a backend-owned lifecycle enum** (`salesStatus`), not derived in the
  ...
  no presale date yet) · `presaleScheduled` · `onSale` · `almostSoldOut` · `soldOut` ·
  `salesClosed` (presale over, event still upcoming) · `cancelled`. `NotPublished` exists only
...
- The Zod-parsed seed is the **single source of event data**; `PROGRAM_EVENTS` is gone
  and the home teaser renders ticketed evenings from the seed (no parade).
...
- **Snapshot moment** is an exported `SEED_SNAPSHOT_AT` (2026-12-01 12:00); the seed mix
  ...
  price, capacity).
- **Teaser rewire:** landing owns `useLandingEventsQuery` (own key, groups pattern) plus
...
- [Area plan](master-plan.md) · [`CONTEXT.md`](../../../CONTEXT.md) (event, session)
- P6 seed pattern: [website master plan](../master-plan.md) → P6 cross-cutting
- [Events teaser](../feature-events-teaser.md) (the existing consumer to rewire)
