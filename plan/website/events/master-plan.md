...
The public **Events area** of the website: the event list, event detail pages,
the ticket order flow ("Kartenwahl" → "Deine Daten" → "Zahlung" → digital ticket) and the
ticket exchange. Expected to become the most-visited part of the site. This file is the area's
...
- **The mocks are inspiration, not spec.** The design README's "READ FIRST" governs; the
  shipped Distilled design system always wins (hard offset-shadows are rejected — sixth
...
- **Mock facts are unverified until the user confirms them.** The mock invents capacity
  ...
  president with a private mobile number (banned — P5/P6 precedent), SMS flows and a
...
- **English identifiers, German copy.** Routes, slugs, IDs, props **and plan file names**
  are English; visible text is German. (The ticket exchange page file is
...
| 2 | [Event list](page-event-list.md) | `/events` | shipped |
| 3 | [Event page](page-event-detail.md) | `/events/$eventSlug` (pinned 2026-08-13) | shipped |
| 4 | [Ticket order flow](page-order-flow.md) | `/events/$eventSlug/order` + `/orders/$orderCode` (pinned 2026-08-18; `…/seats` retired) | shipped |
| 5 | [Purchase & ticket](page-purchase.md) | steps 2–3 inside the ticket order flow + `/orders/$orderCode` | shipped |
| 6 | [Ticket exchange](page-ticket-exchange.md) | `/events/exchange` | shipped (concept) |
...
seed builder + six-evening mid-presale snapshot), the `events` feature scaffold (`api.ts`
hook + German status-label derivations), and the landing "Programm" teaser rewired onto the
seed (Rose Monday parade dropped — the event list is ticketed-only now). Shared UI primitives are
...
trimmed hero card → venue/FAQ/ticket-exchange-teaser/filmstrip + assembly. The shaping session
retired the term **"Programm"** repo-wide (see `CONTEXT.md`); detail-page route pinned as
`/events/$eventSlug`, purchase/ticket-exchange CTAs excluded until their pages exist (each page's
...
pinned the seat-picker and ticket-exchange routes, and established that the **running order
is assembled only 2–3 weeks before an evening** and is published **order-only, never timed**
...
one: the masthead now carries **one** events entry (promoted out of `navItems` into
the CTA), not a "Karten" chip beside a "Veranstaltungen" link.

### E4 — Ticket order flow (shell)
**Status:** shipped (built 2026-08-18)
The order flow per [page-order-flow.md](page-order-flow.md), three slices: routes + ticket-language
rename sweep + entry links → flow shell (stepper, persistent bottom CTA bar, step 2/3
placeholders, `/orders/demo` chain) → step-1 ticket-selection frame (state guard, H1 ladder,
...
step's surface to ticket language (seat selection → ticket selection, "Platz wählen →" → "Karten
wählen →"), extended **Account** to public self-registration (`CONTEXT.md`), and flagged
**seat allocation** as the blocked club decision that keeps step 1's core a placeholder.
...
history-first (`useCanGoBack`); and an honesty test over the flow's copy constants makes the ticket
language non-regressable.

### E5 — Purchase & ticket
**Status:** shipped (built 2026-08-20)
Steps 2–3 of the order flow plus the confirmation page per
[page-purchase.md](page-purchase.md), four slices: order model + seed + order query hook +
`noindex` → step 2 "Deine Daten" (final RHF + Zod buyer form) → step 3 "Zahlung"
(order summary + right-of-withdrawal notice + placeholder payment region) → the confirmation page at
...
and flagged **entry check** as the second blocked club decision (`CONTEXT.md`) — it embargoes
the ticket's whole face, so the confirmation shows the order without any code. As-built notes
...
total price while seat allocation blocks the count, so "Zahlungspflichtig bestellen" stays
...
### E6 — Ticket exchange (concept page)
...
the ticket exchange's values and mechanics as clearly-framed plans in the making (glossary embargo
...
incl. evening swap → "Was wir noch klären" → closing), and left the band + sold-out
...
teaser band stays under the strict ticket-language sweep while the concept copy is swept by its own
...
- Events **key facts** (name/date/location/motto) originate in the Club-App's
  event planner and are published to a public read endpoint; the website never
  edits events.
...
- Sales respect **quota** and the presale window.
...
- **The public read endpoint serves the current session only** (events-list shaping,
  2026-08-13): no past events, no history — past occasions are the gallery's territory.
- **The running order's public face** (event-detail shaping, 2026-08-15): the public read
  endpoint carries **`performers`** — an ordered list of act *names* (groups and guests
  mixed), nullable until the running order is assembled ~2–3 weeks before the evening.
  **Order only, never times.** A per-act group reference is reserved for later; nothing
  links today.
...
- **Album ↔ event linking is by *type*, never by event id** (event-detail shaping,
  2026-08-15): an Album covers an occasion, not necessarily an event (glossary), so
...
- **`orderCode` is a capability URL token** (order-flow shaping, 2026-08-18): the backend
  generates an unguessable random token (≥128 bit entropy) per order; the confirmation
...
- **Public self-registered Accounts** (order-flow shaping, 2026-08-18): buyers may
  optionally self-register a real Account (creates a Person with no Membership) to keep
  mail, ticket overview, history and payment methods; buying never requires one. Member
  onboarding stays invite-only. Duplicate-Person merge is flagged open in `CONTEXT.md`.
- **Seat allocation is an undecided club decision** (order-flow shaping, 2026-08-18):
...
- **Stripe is the single payment provider for ticket sales, embedded** (purchase shaping,
  ...
  bestellen" + summary + total on the final step. Scope is tickets only; other money flows
  stay open. Ledger remains the source of truth; Stripe settles ledger entries.
- **`POST /api/orders`** (purchase shaping, 2026-08-19): creates the order **and** the
  Stripe PaymentIntent from `{ event, selection, buyer { firstName, lastName, email } }`
  (the selection shape is blocked on seat allocation); returns
  ...
  order — no phone, no address, no per-ticket names.
...
- **`GET /api/orders/$orderCode`** (purchase shaping, 2026-08-19; payload amended by the E5
  as-built, 2026-08-20): the order payload — `orderCode`, event identity (**`id`** — the
  cross-sell excludes the ordered evening by it — plus title, date, venue), ticket count + unit
  ...
  order. No reminder mail is promised anywhere.
- **Ticket terms and conditions are an open club/legal fact** (purchase shaping,
  2026-08-19): no terms and conditions exist, so the checkout ships no consent checkbox;
  writing them (plus an `/agb` page) is a **blocker for real online sales**. The
  right-of-withdrawal exemption notice (§312g Abs. 2 Nr. 9 BGB) is frontend-shipped
  already.
- **Entry check is undecided** (purchase shaping, 2026-08-19 — `CONTEXT.md`): the
  digital ticket's face (QR per ticket, PDF, Wallet, name list) is blocked with it; the
  confirmation shows the order without codes until the club decides.
- **The ticket exchange pins no contract** (exchange shaping, 2026-09-01): mechanics *and*
  ...
  round (refund only on actual resale — Ledger territory; waitlist without prepayment,
  mail as the realistic channel; free evening swap ~1 week before; enforcement blocked on
  entry check) lives in
...
- [`CONTEXT.md`](../../../CONTEXT.md) — Event, Running order, Session, Ledger
