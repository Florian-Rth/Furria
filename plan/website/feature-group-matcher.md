---
title: Jeck-Check (Group matcher)
slug: group-matcher
type: capability
status: shipped
...
A **Wahl-O-Mat-style group matcher** on `/join`: the visitor answers eleven questions, every
**group** has answered the same questions in advance, and the page ranks the groups by how well
they agree. It exists because the one thing that keeps people out of a carnival club is not the
fee — it is not knowing where they would fit. The matcher answers that without asking for a
name or an e-mail address.

It is deliberately **not** the mock's "Findomat" (a hardcoded if/else over eight invented groups
that prints one winner). The positions are **authored content per group**, the scoring is a pure,
...
- **Two question roles**, one mechanism, no special cases:
  - **`weighted`** — a thesis the visitor answers **Ja / Neutral / Nein** (or skips). Each group
    holds a stance on it plus an **importance 1–3**. Scored.
  - **`filter`** — a factual question (age band first of all) where each group declares which
    answers it **accepts**. Not scored: a non-accepted answer **excludes** that group from the
...
- **Scoring** (Wahl-O-Mat's proven scale), normalised per group so percentages are comparable:
...
- **Importance is group-owned, not visitor-owned.** The real Wahl-O-Mat lets the *user*
  double-weight theses; we do the opposite on purpose, so a group can say "this one really
...
- A **group decides for itself whether it is currently looking for new members** — its own setting,
  backend-managed later, seeded now as `isRecruiting`. The website shows it.
- **A group that is not recruiting is still ranked normally**, with a badge and the honest line
  that an inquiry is welcome anyway. It is never demoted or hidden: the match is the match, and
  re-sorting would make the ranking lie about the percentage it just printed.
- **There are no open, drop-in trainings** and no per-group training slots — so the result card
  ...
  *"Ansprechpartnerin Katrin Roth"* names a plausible real person in an office, which P5 banned.
  The only next step the result offers is the application or an inquiry.

### Content rules (binding on whoever authors the questions)

- **Every question must split the roster.** A thesis all six groups agree on adds nothing but
...
- **No age band may come back empty.** If a band excludes every group, the result must still say
...
- The **«warum»** panel is **derived** from stances (your answer vs. theirs), never a second body
  of authored copy — 6 groups × 11 questions of quips would rot immediately.
...
- **Every group filtered out** → an honest empty state ("keine Gruppe passt aufs Alter") with the
  human contact, never a blank list.
- **Ties** → stable order, by roster order.
- **Excluded groups are named with their reason**, never silently dropped.
...
- **Result = full ranking**: every eligible group with a percentage bar, #1 emphasised, each
  expandable for the derived «warum», each carrying its recruiting badge. Filtered-out groups
  listed with their reason. Two CTAs: application (with the interest chips prefilled) and the human
  contact.
- **Group data the matcher needs** (all part of the same seed/future `GET /api/groups` payload):
...
- **Its own feature** (`features/group-matcher/`) with its own plan file, separate from
  [Become a member](feature-membership-funnel.md): distinct capability, own future backend
...
- **Built as if the data already came from the backend.** Questions, group positions and the
  groups themselves are destined for the DB, so the feature reads them through **React Query
  hooks in its `api.ts`**, Zod-parsed at the boundary. Until the endpoint exists the `queryFn`
  resolves from a **deletable seed module** (see [Become a member](feature-membership-funnel.md)
...
- **Answers live in `sessionStorage`** (tab-scoped; precedent `preview-access/session-storage.ts`),
  so Back from the application returns to the result instead of question 1, and a mid-quiz refresh does
  not punish the visitor. The **result is always derived, never stored.**
- **Handoff to the application is a URL search param** — `/join/apply?groups=a,b`, `validateSearch` +
...
- **The payload embeds the groups.** `SEEDED_GROUP_MATCHER` is `{ groups, questions }`, so the whole
  feature reads **one** query with one loading/error path instead of a matcher query plus a groups
  query. This changes the deferred contract — `GET /api/group-matcher` must return the groups too, and
...
- **The eleven are 1 `filter` + 10 `weighted`.** The age band is the filter. Elferrat is the one seeded
  group with `isRecruiting: false`, so the badge has two real states. The **content rules are
...
- **The ten theses are deliberate nonsense** (*"Ich erkenne Konfetti mit verbundenen Augen am
  ...
  section's intro **says outright that they are placeholders** — asserted by a test. Changed on the
...
- **The stances were left untouched when the prompts were swapped**, so every scoring, roster-split and
  ...
  says what it actually pins: two opposing answer profiles put a different group on top).
- **A skip is persisted as the marker `"skipped"`** in `sessionStorage` (`furria.kompass.answers`,
  Zod-parsed on read), which is what makes returning from the application land on the *result* rather than
...
- **The all-excluded state is verified through the pure selector** with a synthetic matcher, because the
  ...
  (not a typed `Link`) — see [Become a member](feature-membership-funnel.md) → What shipped.
...
- **The eleven questions and the 66 positions are placeholders and say so on the page.** The ten theses
  ...
  Authoring the real ones means asking the six groups the questions — content work that has to happen
  before the gate comes down, and the binding content rules for it are in **Content rules** above.
- Whether the matcher also earns a place on `/club` next to the group grid (not P6).

## Done When

- A visitor answers eleven questions on a phone or a desktop, in light or dark, and gets a ranked,
  explainable list of groups that never contains an age-impossible match — then lands on the
  application with their interests already ticked.

## References

- `CONTEXT.md` (**Group** — Layer B, many-to-many, independent of **membership type**).
- Mock: `docs/design/join-page/` (`Findomat` — the idea, not the implementation).
- Sibling feature: [Become a member](feature-membership-funnel.md).
