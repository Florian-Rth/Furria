...
type: capability
status: shipped
mock: docs/design/club-page/
adrs: []
---

## What & Why

The public **club** page (`/club`, nav label **Verein**): who the FCC is, for a visitor —
especially a prospective member. A single scrolling editorial page in the "Konfetti Kinetik"
brand that tells the club's story, teaches the session, showcases the **groups**, and puts faces
to the **offices** — ending on a "Mitglied werden" call. It is the trust-building page the hero and
the Join-in band point at.
...
1. **Hero** — giant derived session numeral + ribbon (desktop) + two-tone headline + CTAs + framed
   photo & 11.11 seal.
2. **Chapter 01 — Story + stat strip** ("Ein Verein mit Herz").
3. **Carnival call band** — the single mid-page red strip ("GROSS FURRIA!").
4. **Chapter 02 — Chronicle** — vertical-spine timeline (placeholder milestones).
5. **Chapter 03 — One session** — the 11.11.→Ash Wednesday season arc (real content).
6. **Chapter 04 — Groups** — auto-fill tile grid → detail **modal** (the page's one interaction).
7. **Chapter 05 — People** — portrait wall keyed on **offices** (placeholder faces).
...
- **Seam (deferred, NOT scaffolded — YAGNI):** later the Club-App backend supplies groups,
  offices/people, and a per-office "show on public page" flag, plus real member/group counts and photos.
...
### Design language (Distilled wins; mock is inspiration)
- The mock's **"poster chapter"** system (hard offset-shadows as the leading elevation, square
  corners, 2px ink borders everywhere) is **rejected** as the system. We keep our shipped
  **"Distilled"** language: MUI `Card`, `radius.base` (14), hairline `divider` borders, `shadow.rest`
...
- **No dark ink-panels.** The mock's alternating cream/ink chapters are dropped; every section sits
  on the normal theme background. **Red appears only as two full-bleed strips** (carnival call +
...
### Story + stats (Chapter 01)
- Keep the pull-quote (generic-true). Body paragraph ships as **clearly-flagged placeholder** (typed
  content const) — the club replaces real club history before launch. Framed photo alongside.
...
### Carnival call band (the single mid-page red strip)
...
### Season arc (Chapter 03)
- ChapterHeader + intro + 5 MUI `Card`s (11.11. opening · Advent rehearsals & setup · February
  gala sessions · Rose Monday parade · Ash Wednesday wind-down), first accented red. Content is
  **real** (generic-accurate carnival structure), not placeholder-flagged. Reinforces the **session**
  term (see `CONTEXT.md`).

### Groups showcase (Chapter 04) — the centerpiece
...
- **Amended in P6 (2026-07-30):** the roster is no longer owned here. `GROUPS` is now built from the
  ...
  (`blurb`/`memberMeta`/`fullText`/`lead`) keyed by group id in `GROUP_EDITORIAL`; key completeness is
...
- Typed `Group` interface + editable `GROUPS` const. **Shipped field names are English**
  ...
  public-facing labels only; the domain's trainer office / training planner live in the Club-App.
...
- `/join` needs the same groups and features may never import each other, so the **roster moves to
...
- **`/club` keeps reading it synchronously and is deliberately *not* migrated to React Query.** Its
  group list is indexable content and P7 prerenders every route — fetching would blank the section
...
### People wall (Chapter 05) — offices, not "Vorstand"
- The section presents the people who hold the club's public **offices** ("die Gesichter"). Title
  *"MENSCHEN, DIE FURRIA SIND"* (people-focused; cleanly avoids the forbidden "Vorstand" framing —
  see `CONTEXT.md`). Each portrait's red eyebrow = the person's **office**. **No "Vorstand" in code or
...
### Chapter header
- Club-local `ChapterHeader` (reused by Story/Chronicle/Season/Groups/People): giant **red Anton
...
- Full-bleed red band sharing the red-surface + `KkBroomMark` idiom with `MitmachenBand` (visual CI)
  ...
  AB 30 € IM JAHR · KINDER 15 €** (real fee tiers), Anton H2 **WERDE TEIL VON FURRIA**, intro.
...
- **Office** = fixed set, **group** = performing unit, **session** = the season. No "board
...
- **None blocking.** All three original open questions resolved:
  ...
  - *Which offices public, names/photos or roles?* → **placeholder faces keyed on offices** for v1;
    real names/photos + per-office public-visibility flag come with the Club-App backend (seam noted,
    not scaffolded).
- **Deferred (need real content / Club-App backend):** real club history + chronicle milestones,
  real groups/offices/photos, real member & group counts, live data.

## Done When

- `/club` renders the full editorial page (hero → story+stats → carnival call → chronicle → season →
  groups+modal → people → recruit) as one responsive page, light + dark, tokens only.
- The group tiles open an accessible, motion-animated detail modal (reduced-motion aware).
...
3. **Carnival call band** (red strip #1). Full-bleed red band, GROSS/FURRIA! treatment, `KkBroomMark`
   watermark.
   *Delivers (FE):* the red carnival call band in the page.
   *Verify:* copy + carnival-call law, watermark present, tokens only, both themes.
4. **Chronicle timeline.** `ChapterHeader` (reused) + vertical-spine timeline from typed `MILESTONES`
   (placeholder), MUI cards + dots, first accented.
   *Delivers (FE):* the chronicle section.
...
6. **Groups grid (no modal).** `ChapterHeader` + auto-fill tile grid from `GROUPS` + numbered
...
7. **Groups detail modal (motion).** Make tiles open the MUI `Modal` + `motion.div` panel
...
8. **People wall (offices).** `ChapterHeader` + portrait grid from typed `PEOPLE` (`amt/name/tint`),
   soft framed portraits.
   *Delivers (FE):* the faces section.
   *Verify:* portraits from the array; office eyebrow; **no "Vorstand"** in output; responsive; themes.
9. **Recruit band** (red strip #2). Full-bleed red closing band, two buttons (→ `/join`,
   ...
   *Verify:* copy + fee tiers, both CTAs navigate, watermark; `ClubPage` test asserts all
   sections present in order at both breakpoints; `pnpm build`/`typecheck` pass.

## References

- Mock: `docs/design/club-page/` (`src/fcc-ds-club-v2.jsx` → `ClubV2`; README = the handoff).
  **Inspiration for layout/content only — Distilled theme + `@furria/ui` primitives are binding.**
- `CONTEXT.md` (office, group, session, fee, carnival call). `lib/club.ts` (derived facts).
