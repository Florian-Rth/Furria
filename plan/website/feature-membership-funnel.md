...
The **Become a member** funnel: turns an interested visitor into a **membership application** that
reaches the club. Destination of the home [join-in band](feature-join-in-band.md) and of the
recruit band on `/club`. It explains what membership costs and means, and it captures the
application.

The page's thesis — taken from the mock and kept, because it is right — is that what keeps people
out of a carnival club is **not** the fee but the fear of not fitting in and of not being good
enough. Hence the headline **"DU MUSST NICHT TANZEN KÖNNEN."** and hence the
...
- `/join/apply` — the application form, and the confirmation rendered in its place on success.
...
- The application posts to **our own API** (`POST /api/membership-applications`), which persists it
  and mails the club. No third-party form service, no `mailto:`-only path, no client-side PDF. The
  ADR carries the full reasoning and the consequences; do not restate them here.
...
- **P6 is frontend-only, and the backend is deferred — not scheduled.** The contract lives in
  [master-plan → Deferred → Membership-application and group
  backend](master-plan.md#membership-application-and-group-backend-not-scheduled). Until it exists
  the mutation calls the real URL, gets a 404, and the form shows its normal error state — which
  **always** offers the human fallback (email). Nothing fake, nothing disabled, and the frontend
  needs zero changes on the day the endpoint lands. Only testers can see it: the preview gate is up
  until P7.
...
- Questions, group positions and the groups themselves are all **destined for the DB**. So the
...
- **`/club` reads the same seed synchronously and is not migrated to queries.** Its group list is
...
- **`Passiv` is not a membership type.** The domain expert overturned the 2026-07-16 note during
  this shaping: `Passiv` was the word for a membership that **pauses for a session**, which is a
  **status**. Type is now **Aktiv / Jugend / Ehren**; status is **aktiv / paused / beendet**.
...
- **Only Aktiv and Jugend are joinable.** **Ehren** is conferred and is **not published on the
  public website** — the mock's line about the general meeting conferring it is dropped.
- **The membership type is derived from the date of birth, never asked.** The mock lets you tick
...
- **There are no open, drop-in trainings.** Nobody can simply turn up. This kills the mock's entire
  *"ERST VORBEIKOMMEN, DANN ENTSCHEIDEN"* premise, its dated open-training list, and the
  ...
  is now the Jeck-Check (costs nothing, asks for nothing) and an **inquiry**, which is always
  welcome.
- **Groups are independent of membership.** Person↔group is many-to-many and a member in no group
  is normal — hence the application's group field is **optional and multi-select**, framed as
...
- **No "Vorstand", in code or copy** (glossary law, and P4 already fixed this twice). The mock's
  *"Vorstand nimmt auf"*, *"Der Antrag geht direkt an den Vorstand"* and *"spricht mit dem
  ...
  name correctly.
- **Applicant ≠ member.** The confirmation does not say "WILLKOMMEN" — the club has not decided
  yet. New glossary term **membership application** in `CONTEXT.md`.
- **Costumes are not (all) paid by the club.** The mock's *"Kostüme gestellt oder bezuschusst"*
...
- **Hero.** Eyebrow `MITGLIED WERDEN · SESSION 2026/27`, H1 **DU MUSST NICHT TANZEN KÖNNEN.**,
  lead, and **application primary / Jeck-Check secondary** — the page has one job, and the visitor who
...
- **The Ticket** — the page's one signature object. A **gold** surface with ink text (gold is the
  ...
  **`MITGLIED NR. ____`** line instead of the mock's fake "NR. 185". `useReducedMotion`-aware.
  - It is an **info flyer in the shape of a ticket** — the one-glance answer for someone
    considering joining — not a purchase and not a trial ticket. Rows: **BEITRAG** (30 € / bis 17
...
- **Kontakt** — **one official channel**, `CLUB_CONTACT_EMAIL`. The mock's three named people with
  private mobile numbers are cut: P5 already banned inventing plausible real people in an office,
  and publishing private mobiles is a spam and GDPR problem even when the people are real.
- **No helper list.** Its target does not exist (P5's dead-link precedent), and the need it served
  — *unterstützen, ohne mitzumachen* — has no separate home in the model.
- **No group showcase section.** The Jeck-Check result lists them; `/club` owns the showcase. One
  link, no third roster surface.
- **No landing teaser.** `/` is static-final and P4 spent its one slot on news; the
  join-in band already points here.

### `/join/apply` — the application
...
- **Fields:** Vorname\*, Nachname\*, Geburtsdatum\*, Straße\*, PLZ\*, Ort\*, E-Mail\*, Telefon,
  ...
  Club-App will eventually own; the mock's city-only "Wohnort" is useless for a club's records.
- **Derived, not asked:** membership type and fee (from date of birth), shown back live.
- **Under 18 → a required guardian block** (Name\* + E-Mail oder Telefon\*), appearing when the
  ...
  cannot validly consent to joining a club alone (§107 BGB). The mock's promised **SMS
...
- **Consent: one unchecked, required checkbox** — bylaws and privacy notices. Two mock defects
  fixed: it **defaults to checked** (legally invalid) and it **bundles photo consent** into the
  same box (bundling ban) on a topic `CONTEXT.md` explicitly flags as unresolved. **No photo
  consent here** — it belongs to whoever eventually designs the photo permission.
- **The bylaws are linked** even though no `/satzung` route exists yet, accepting the branded 404
...
Pure functions, unit-tested directly, never through rendered mock data: age → membership type
→ fee derivation (incl. the birthday-today boundary), the search-param parser, and the form
...
- **The application route file is `join_.apply.tsx`** — the trailing underscore, exactly P4's finding: the
...
- **The POST body carries no membership type and no fee.** Both stay derived on the server from
  ...
  invented.
- **The groups come from a `useGroupsQuery`** in the feature's `api.ts` resolving from
...
- **Only the hero's application CTA is a typed `Link`.** The Jeck-Check result CTA stays a plain href:
...
- Retention/deletion rule for applicant data in the privacy policy — lands with the deferred
...
- Every membership fact on the page (30 €/15 €, keine Aufnahmegebühr, Kündigung zum Sessionende,
  Ruhen, who decides an Aufnahme) is **unverified against the bylaws**. Accepted for now; a P7
  launch task.

## Done When

- A visitor on a phone or a desktop, in light or dark, understands what membership costs and means,
  can find where they might fit, and can submit an application that either reaches the club or fails
  honestly into a human fallback — with a confirmation that does not call them a member yet.

## Implementation plan (phases)

Ten vertical slices, each leaving the app building and working. **Frontend only** — the backend is
[deferred](master-plan.md#membership-application-and-group-backend-not-scheduled). Review each slice with
...
1. **Seed module + schemas.** `src/lib/seed/groups.ts` shaped like `GET /api/groups`
   ...
   groups. `/club`'s `groups-content.ts` reads it for the roster and keeps its editorial copy keyed
...
3. **`/join` page shell + hero.** `PageLayout` + `KkHeroSection`, replacing `PlaceholderPage`;
   headline, lead, application-primary/Jeck-Check-secondary CTAs, three derived stats via `KkStatRow`,
...
9. **`/join/apply`.** Route + form: RHF + Zod, derived type/fee summary, conditional guardian
...
- `CONTEXT.md` (**Membership**, membership type (retired), **Membership pause**,
  **Membership application**, **Group**, **Account**, **Invitation**, **Fee**).
...
- Sibling: [Jeck-Check](feature-group-matcher.md). Entry point:
  [join-in band](feature-join-in-band.md).
