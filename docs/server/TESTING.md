...
2. **`DatabaseResetService`** — owned reset instead of Respawn: one `TRUNCATE … CASCADE` over
   ...
   `RolePermission`, `RoleHolding` — parents before children) and the club's migration-seeded
   Stammdaten (`Session`, `Venue`) after every truncate.
...
1. **`ApiTestFixture.BuildAsync(Action<SeedContextBuilder>)`** — the single seed entry point:
   ...
   typeof(Session), typeof(Venue)]`,
   in that FK-safe order, and the admin *and her Admin Rolle* survive every reset. Two constraints
   this imposes on production code: those tables must keep Npgsql's default
   ...
   `Session` and `Venue` are in the list for the same reason the admin is: they are *Stammdaten*
   every environment ships, written by a migration's `InsertData` rather than by a test — the
   2026/27 Session and the three Orte the club actually has. Truncating them would mean a fresh
   production database and a test database disagree about what "empty" is. The price is stated
   below: the seeded values are reserved, and no test may seed over them.
...
3. **`AliasRegistry`** — `ctx.Identity.People.IdOf("alice")`; an unknown alias throws listing the
   ...
   `ctx.Roles.{Roles, RolePermissions, RoleHoldings}` and `ctx.Club.Sessions`. A `RolePermission` has no alias of its own —
...
- **Only `person`, `account`, `role`, `role_permission`, `role_holding`, `session` and `venue`
  survive a reset.** Everything else, the three Gruppen tables included, is truncated before every
  `BuildAsync`, so a test seeds every Gruppe it needs and may never assume one from a neighbour.
- **The seeded Stammdaten are reserved — never seed over them.** The snapshot carries the
  migration's own rows into every test: the Session with `start_year = 2026`
  (Motto `FURRIA — Der Mittelpunkt des Universums`, `number` and `signet_svg` `NULL`) and the
  three Orte `Sporthalle`, `Vereinsraum` and `Lager` (`sort_order` 1, 2, 3). So:
  **no test may call `AddSession(_, 2026, …)`, and none may call `AddVenue(_, "Sporthalle" |
  "Vereinsraum" | "Lager", …)`** — `ix_session_start_year` / `ix_venue_name` would reject it with
  a unique violation out of `BuildAsync` that reads like a bug in the code under test.
  `SessionPersistenceTests` therefore works in 1984/2021/2022, `VenuePersistenceTests` in
  *Bühnenhaus*. The flip side is the one thing a test may assume from the fixture besides the
  admin: those rows are *there*. A test that wants an empty `venue` table does not exist — assert
  Ort lists by name membership, never by a raw `Count` or `Assert.Empty`.
