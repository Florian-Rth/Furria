...
2. **`SeedContextBuilder` / `IdentitySeedBuilder` / `RoleSeedBuilder`** — pure accumulators.
   One sub-builder per bounded context, not per entity: `builder.Identity(…)` takes `AddPerson`,
   `AddPersonContact`, `AddMembership`, `AddMembershipPause`, `AddFeeReduction`, `AddAccount`;
   `builder.Roles(…)` takes `AddRole`, `AddRoleWithDetails`, `AddRoleHolding`, `AddRoleWithHolder`.
   ...
   → Role → RolePermission → RoleHolding, one `SaveChanges` per layer, and uniquifies emails
...
3. **`AliasRegistry`** — `ctx.Identity.People.IdOf("alice")`; an unknown alias throws listing the
   ...
   `ctx.Identity.{People, Memberships, Pauses, FeeReductions, Accounts}` and
   `ctx.Roles.{Roles, RolePermissions, RoleHoldings}`. A `RolePermission` has no alias of its own —
...
5. **`Expected` DSL** — `ctx.Expected.Account(id).ToHaveEmail(…)` enqueues deferred reads; one
   ...
   `Expected.RoleHoldingsOfPerson(personId)` (`ToHaveCount`, `ToHaveOpenCount`).
