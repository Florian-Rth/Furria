...
public sealed class MetAnalyzerTests
{
    ...
    [Fact]
    public async Task Should_FlagDanglingAlias_When_ReferenceHasNoMatchingDeclaration()
    {
        const string source = """
            namespace N
            {
                class B
                {
                    public B AddUser(string alias) => this;
                    public long IdOf(string alias) => 0;
                }
                class C
                {
                    void M()
    ...
    [Fact]
    public async Task Should_FlagDanglingAlias_When_ClientForAsyncReferenceHasNoDeclaration()
    {
        const string source = """
            namespace N
            {
                ...
                {
                    public B AddUser(string alias) => this;
                    public System.Threading.Tasks.Task<object> ClientForAsync(string alias) =>
                ...
                class C
                {
                    void M()
                    {
                        var b = new B();
                        b.AddUser("alice");
                        var client = b.ClientForAsync("alicee");
                    }
                }
            }
            """;
    ...
    [Fact]
    public async Task Should_NotFlagAlias_When_DeclarationExists()
    {
        const string source = """
            namespace N
            {
    ...
    [Fact]
    public async Task Should_NotFlagAlias_When_DeclaredInAnotherMethodOfSameClass()
    {
        const string source = """
            namespace N
            {
                ...
                class C
                {
                    void Arrange(B b) => b.AddUser("alice");
                    void Act(B b)
                    {
                        var id = b.IdOf("alice");
                    }
    ...
    [Fact]
    public async Task Should_NotFlagAlias_When_ClassOptsOutWithSeedsAliases()
    {
        const string source = """
            namespace N
            {
                class SeedsAliasesAttribute : System.Attribute { }
                class B
                {
                    public long IdOf(string alias) => 0;
                }
