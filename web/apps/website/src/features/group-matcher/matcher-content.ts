...
export const matcherKicker = 'JECK-CHECK';
export const matcherTitle = 'WO PASSE ICH HIN?';

export const matcherIntro =
  'Elf Fragen, keine Anmeldung, kein Name, keine E-Mail-Adresse. Am Ende steht eine Rangliste: welche Gruppe zu dir passt — und vor allem, warum. Was dir nichts sagt, überspringst du; das kostet Genauigkeit, aber keine Punkte. Die Fragen sind derzeit Platzhalter und frei erfunden.';

export interface MatcherStanceChoice {
  value: Stance;
  label: string;
}
...
export const matcherStanceLabels: Record<Stance, string> = {
  yes: 'Ja',
  neutral: 'Neutral',
  no: 'Nein',
};

export const matcherStanceChoices: MatcherStanceChoice[] = stanceOrder.map((value) => ({
  value,
  label: matcherStanceLabels[value],
}));

export const matcherImportanceLabels: Record<Importance, string> = {
  3: 'der Gruppe besonders wichtig',
  2: 'der Gruppe wichtig',
  1: 'für die Gruppe nebensächlich',
};

export const matcherAgreementLabels: Record<MatchAgreement, string> = {
  agree: 'Einig',
  partial: 'Teilweise einig',
  disagree: 'Uneinig',
};

export const matcherLabels = {
  back: '← zurück',
  skip: 'überspringen',
...
export const buildProgressLabel = (index: number, total: number): string =>
  index >= total ? matcherProgressDoneLabel : `Frage ${index + 1} von ${total}`;
...
export const buildReasonAnswerLine = (reason: MatchReason, groupName: string): string =>
  `Du: ${matcherStanceLabels[reason.answer]} · ${groupName}: ${matcherStanceLabels[reason.stance]} · ${matcherImportanceLabels[reason.importance]}`;
...
export const resolveRecruitingBadge = (isRecruiting: boolean): MatcherRecruitingBadge =>
  isRecruiting
    ? {
