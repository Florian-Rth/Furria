...
import { matcherMailHref } from '@/features/group-matcher/matcher-content';
import { SEEDED_GROUP_MATCHER } from '@/lib/seed/group-matcher';
import { renderWithProviders } from '@/test/render';
import { GroupMatcherSection } from './GroupMatcherSection';
...
describe('GroupMatcherSection', () => {
  it('names itself the Jeck-Check and never an -O-Mat', async () => {
    renderWithProviders(<GroupMatcherSection />);

    expect(
      await screen.findByRole('heading', { level: 2, name: 'WO PASSE ICH HIN?' }),
    ).toBeInTheDocument();
    expect(screen.getByText('JECK-CHECK')).toBeInTheDocument();
  ...
  it('keeps the answers through a reload of the page', async () => {
    const user = userEvent.setup();
    const first = renderWithProviders(<GroupMatcherSection />);
    await findPrompt(firstPrompt);
    await answerCurrent(user, 0);
    await findPrompt(secondPrompt);
    first.unmount();
  ...
  it('forgets every answer when the visitor starts over', async () => {
    ...
    expect(await findPrompt(firstPrompt)).toBeInTheDocument();
    expect(window.sessionStorage.getItem('furria.matcher.answers')).toBeNull();
  });
});
