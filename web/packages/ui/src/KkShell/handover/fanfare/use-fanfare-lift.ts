...
import { crossingOf, FANFARE_THRESHOLD, liftAt } from './fanfare-score';

export interface FanfareLiftMotion {
...
export const useFanfareLift = (): FanfareLiftMotion => {
  const { scrollY } = useKkShellScroll();
  const visible = useMotionValue(scrollY.get() >= FANFARE_THRESHOLD ? 0 : 1);
