...
export interface SwayLetterPlan {
  upper: string | null;
  lower: string | null;
  homeLeft: number;
  homeTop: number;
...
export interface SwayPlan {
  letters: SwayLetterPlan[];
  dockScale: number;
}

export interface SwayLetterPose {
  x: number;
  y: number;
  scale: number;
  presence: number;
...
export const swapsCase = (letter: SwayLetterPlan): boolean =>
  letter.upper !== null && letter.lower !== null;

export const lowerMixOf = (letter: SwayLetterPlan, lowered: boolean): number => {
...
const presenceOf = (letter: SwayLetterPlan, progress: number): number => {
  if (letter.lower === null) {
...
export const restPoseAt = (progress: number, index: number, count: number): SwayLinePose => {
...
export const arrivalPoseAt = (progress: number, index: number, count: number): SwayLinePose => {
  const arriving = ramp(progress, ARRIVAL_FROM, 1);
  const lead = count > 1 ? (index / (count - 1)) * ARRIVAL_STAGGER : 0;
  const own = ramp(arriving, lead, lead + 1 - ARRIVAL_STAGGER);
  const ahead = 1 - own;

  return { x: ARRIVAL_SHIFT * ahead * ahead, opacity: own };
};

const firstLineOf = (glyphs: readonly SwayGlyph[]): SwayGlyph[] =>
  glyphs.filter((glyph) => glyph.line === 0);

export const swayPlanOf = (
  headline: readonly SwayGlyph[],
  headlineOrigin: SwayPoint,
  title: readonly SwayGlyph[],
  slotOrigin: SwayPoint,
  dockScale: number,
): SwayPlan => {
  const docked = firstLineOf(title);
  const count = Math.max(headline.length, docked.length);
  const letters: SwayLetterPlan[] = [];
