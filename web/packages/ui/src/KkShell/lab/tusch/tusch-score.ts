...
export type TuschStrike = 'impact' | 'quiet';

export type TuschCrossing = 'down' | 'up' | null;

export interface TuschLift {
  hold: number;
  scale: number;
  shadowX: number;
  shadowY: number;
...
export interface TuschPlacement {
  x: number;
  y: number;
  scale: number;
}

export interface TuschHeadlineBox {
  left: number;
  top: number;
  width: number;
  height: number;
...
export interface TuschGeometry {
  headline: TuschHeadlineBox | null;
  slot: TuschSlotBox;
  copyGlyph: number;
}

export interface TuschPlan {
  from: TuschPlacement;
  to: TuschPlacement;
  shadowX: number;
  shadowY: number;
...
export const TUSCH_THRESHOLD = kkTokens.shell.scrollTravel / 2;
...
export const TUSCH_BEATS: readonly Stop[] = [
  [0.15, 1],
  [0.32, 0.58],
  [0.49, 0.32],
];

const TIMING: Record<TuschStrike, TuschTiming> = {
...
const BOUNCE: Record<TuschStrike, readonly Stop[]> = {
  impact: [
    [0, 1],
    [0.16, 0.93],
    [0.48, 1.04],
...
const KNOCK: Record<TuschStrike, TuschPlacement & { rotate: number }> = {
...
export const liftAt = (scrollOffset: number): TuschLift => {
  const progress = clamp01(scrollOffset / TUSCH_THRESHOLD);
  const lift = easeOut(progress);
  const leaving = clamp01(scrollOffset / (TUSCH_THRESHOLD * LEAVE_SHARE));

  return {
    hold: TUSCH_THRESHOLD * progress,
...
export const crossingOf = (previous: number, next: number): TuschCrossing => {
  if (previous < TUSCH_THRESHOLD && next >= TUSCH_THRESHOLD) {
    return 'down';
  }
...
export const contactOf = (strike: TuschStrike): number =>
  TIMING[strike].anticipate + TIMING[strike].slam;

export const strikeDurationOf = (strike: TuschStrike): number => {
  const lastBeat = TUSCH_BEATS[TUSCH_BEATS.length - 1]?.[0] ?? 0;
...
const slotPlacementOf = (slot: TuschSlotBox, glyph: number): TuschPlacement => {
...
const flightOf = (time: number, plan: TuschPlan): TuschPlacement => {
...
export const ringAt = (since: number): number =>
  Math.max(0, ...TUSCH_BEATS.map((beat) => pulseOf(since, beat)));
...
export const strikeFrameAt = (time: number, plan: TuschPlan): TuschFrame => {
...
export const buzzPatternOf = (contactSeconds: number): number[] => {
  ...
  const hits = [0, ...TUSCH_BEATS.map(([peak]) => Math.round(peak * 1000))];
