...
import { KonfettiDockChip } from './KonfettiDockChip';

const LAYER_DEPTH: Record<DockChipLayer, number> = { behind: -1, front: 1 };

interface KonfettiDockBurstLayerProps {
  layer: DockChipLayer;
...
export const KonfettiDockBurstLayer: FC<KonfettiDockBurstLayerProps> = ({
  layer,
  chips,
  left,
  top,
  ...
  const layerChips = chips
    ...
    .map((chip) => <KonfettiDockChip key={chip.id} chip={chip} spread={spread} />);
