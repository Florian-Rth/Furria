...
export const useKonfettiDock = (): KonfettiDock => {
  ...
  const follow = (scrollOffset: number): void => {
    ...
    const stage = dockStageAt(progress, current !== null, headerDrift);
    ...
    if (current !== null) {
      const flight = dockFlightAt(scrollOffset, current, DOCK_TRAVEL);
  ...
  const settle = useEffectEvent((): void => {
    previousRef.current = dockProgressAt(window.scrollY, DOCK_TRAVEL);
