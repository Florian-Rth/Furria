import type { SchunkelGlyph, SchunkelPlan, SchunkelPoint } from './schunkel-pose';
import { schunkelPlanOf } from './schunkel-pose';

const HEADLINE_SELECTOR = '[data-kk-schunkeln-header] [data-kk-screen-header-title]';
...
const glyphsOf = (element: Element | null): SchunkelGlyph[] => {
  if (element === null) {
    return [];
  }
...
const documentOffsetOf = (element: HTMLElement): SchunkelPoint => {
  let left = 0;
  let top = 0;
...
const viewportOffsetOf = (element: Element): SchunkelPoint => {
...
export const measureRestGlyphs = (cell: HTMLElement | null): SchunkelGlyph[] => {
...
export const measureSchunkelPlan = (slot: HTMLElement | null): SchunkelPlan | null => {
