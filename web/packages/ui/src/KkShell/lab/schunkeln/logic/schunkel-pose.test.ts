...
import type { SchunkelGlyph, SchunkelLetterPlan } from './schunkel-pose';
import {
  handoverProgressAt,
  letterPoseAt,
  letterProgressAt,
...
const glyph = (char: string, left: number, line = 0): SchunkelGlyph => ({
  char,
  left,
  top: 0,
  width: 10,
...
describe('lowerMixOf', () => {
  const letter = (upper: string | null, lower: string | null): SchunkelLetterPlan => ({
    upper,
    lower,
    homeLeft: 0,
    homeTop: 0,
...
describe('letterPoseAt', () => {
  const letter: SchunkelLetterPlan = {
    upper: 'M',
    lower: 'M',
    homeLeft: 20,
    homeTop: 100,
